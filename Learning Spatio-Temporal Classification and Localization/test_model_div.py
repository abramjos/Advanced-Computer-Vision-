...
#model = C3D().cuda()
model = res_34.model(pretrained_path='models/res_34_features.pth', fix_weights= True)
#model.cuda()
...
if args.test_flag != True:
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (clips,boxes,labels) in enumerate(trainloader):
            ...
            conf, loc = model(clips)
            conf_flip, loc_flip = model(clips_flipped)
...
#            import ipdb;ipdb.set_trace()
        ...
        if float(loss.data) < best_loss:
            ...
            torch.save(model.state_dict(), '%s/set_test_res_34_sgd_%d_%f.pth'%(args.save_folder,epoch+1,float(loss.data)))
    ...
    model.load_state_dict(torch.load(pertained_weights))
